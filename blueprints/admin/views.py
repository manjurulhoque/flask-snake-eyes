...
from blueprints.admin.models import Dashboard
from blueprints.user.decorators import role_required
...
def users(page):
    ...
    if request.args.get('q'):
        query = query.filter(User.search(request.args.get('q', '')))

    paginated_users = query.order_by(User.role.asc(), text(order_values)) \
